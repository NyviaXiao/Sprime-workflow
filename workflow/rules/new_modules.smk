...
rule report:
    input:
        ...
        adaptive_segments='adaptive/segment_summary.tsv' if C['adaptive']['enabled'] else [],
        adaptive_top2='adaptive/top2_candidates.tsv' if C['adaptive']['enabled'] else [],
        adaptive_shared='adaptive/shared_top2_regions.tsv' if C['adaptive']['enabled'] else [],
        affinity_tables=expand('affinity/{population}/{ref}.tsv.gz', population=P, ref=list(R)) if C['affinity']['enabled'] else [],
