...
CLASSIFICATION_COLUMNS = ['population', 'archaic_class', 'n_segments', 'genome_coverage_mb']
INDIVIDUAL_COLUMNS = ['population', 'archaic_class', 'per_individual_introgressed_mb',
                      'n_introgressed_individuals']
GMM_COLUMNS = ['population', 'reference', 'selected_components',
               'adjusted_p_1_vs_2', 'adjusted_p_second']
ADAPTIVE_COLUMNS = ['population', 'adaptive_group', 'adaptive_rank', 'chromosome',
                    'candidate_start', 'candidate_end', 'core_mean_AF',
                    'core_variant_count']
...
def _table(rows, fields=None):
    ...
    body = []
    for row in rows:
        cells = [f'<td class="{"text" if field in text else "numeric"}">{html.escape(_format_value(row.get(field, "")))}</td>' for field in fields]
        body.append('<tr>' + ''.join(cells) + '</tr>')
    return f'<div class="table-wrap"><table class="compact"><thead><tr>{header}</tr></thead><tbody>{"".join(body)}</tbody></table></div>'
...
def _settings_rows(cfg):
    ...
    return [{'setting': f'{module}: {setting}', 'value': value} for module, setting, value in rows]
...
def build_report(output_html, output_pdf, run_manifest, software_versions, resolved_config,
    ...
    cards = ''.join(f'<div class="card"><span>{html.escape(label)}</span><strong>{html.escape(str(value))}</strong></div>' for label, value in (
        ('Genome build', manifest.get('genome_build', '')), ('Populations', ', '.join(manifest.get('populations', []))), ('Archaic references', len(manifest.get('archaic_references', []))), ('Generated', manifest.get('run_time', ''))))
    ...
    sections = [
        ('overview', 'A. Run Overview', '<p>Run metadata identify this analysis without embedding filesystem paths or the complete resolved configuration.</p><p><b>Genome build:</b> ' + html.escape(str(manifest.get('genome_build', 'NA'))) + ' &nbsp; <b>Populations:</b> ' + html.escape(', '.join(manifest.get('populations', []))) + ' &nbsp; <b>Chromosomes:</b> ' + html.escape(', '.join(manifest.get('chromosomes', []))) + ' &nbsp; <b>Archaic references:</b> ' + html.escape(', '.join(manifest.get('archaic_references', []))) + '</p><h3>A2. Analysis Settings</h3>' + _table(_settings_rows(cfg))),
        ('classification', 'B. Classification Results', '<p>' + _classification_narrative(class_rows) + '</p>' + _table(class_rows, CLASSIFICATION_COLUMNS) + (_figure('Classification genome coverage', classification_plot, 'Figure B1. Marker-supported genomic coverage by final segment classification.', outdir) if classification_plot else '')),
        ...
        ('individual', 'F. Individual-level Introgression', '<p>' + _individual_narrative(individual_rows, enabled.get('individual', False)) + '</p>' + (_table(individual_rows, INDIVIDUAL_COLUMNS) + (_figure('Per-individual archaic haplotype amount', individual_plot, 'Figure F1. Mean marker-supported archaic haplotype sequence amount per target individual.', outdir) if individual_plot else '') if individual_rows else '')),
