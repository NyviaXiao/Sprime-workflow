...
def build_report(output_html, output_pdf, run_manifest, software_versions, resolved_config,
    ...
    sections = [
        ('overview', 'A. Run Overview', '<p>Run metadata and software versions identify this analysis without embedding filesystem paths or the complete resolved configuration.</p><p><b>Workflow commit:</b> ' + html.escape(str(manifest.get('git_commit', 'NA'))) + ' &nbsp; <b>Genome build:</b> ' + html.escape(str(manifest.get('genome_build', 'NA'))) + ' &nbsp; <b>Populations:</b> ' + html.escape(', '.join(manifest.get('populations', []))) + ' &nbsp; <b>Chromosomes:</b> ' + html.escape(', '.join(manifest.get('chromosomes', []))) + ' &nbsp; <b>Archaic references:</b> ' + html.escape(', '.join(manifest.get('archaic_references', []))) + '</p><h3>A2. Analysis Settings</h3>' + _table(_settings_rows(cfg)) + '<h3>Software environment</h3>' + _table(software)),
        ...
        ('provenance', 'I. Reproducibility & Provenance', '<p>Detailed runtime environment, input/output information, and provenance files are stored in the <code>provenance/</code> directory.</p>'),
    ]
