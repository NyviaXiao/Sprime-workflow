...
def resolve_config(path, overrides):
    ...
    # Explicit compatibility translation for configurations written before
    # the optional reporting branches were introduced.  The sample config
    # enables them; old analyses remain reproducible with those branches off.
    plots = c.setdefault('plots', {})
    if 'contour' not in plots and 'contour_pairs' in plots:
        plots['contour'] = {'enabled': True, 'mode': 'explicit_pairs', 'pairs': plots.pop('contour_pairs')}
    for key in ('landscape', 'affinity', 'adaptive', 'report'):
        c.setdefault(key, {'enabled': False})
    c['adaptive'].setdefault('min_allele_frequency', 0.30)
    c['adaptive'].setdefault('max_frequency_drop', 0.20)
    c['adaptive'].setdefault('min_callable_sites', 10)
    c['adaptive'].setdefault('neanderthal_match_threshold', 0.50)
    c['adaptive'].setdefault('denisovan_match_threshold', 0.40)
    c['adaptive'].setdefault('top_regions', 2)
    ...
    if c['report']['enabled']:
        if not isinstance(report_pair, list) or len(report_pair) != 2:
            raise ValueError('report.contour_pair requires one pair')
        ...
        if contour['enabled']:
            ...
            if tuple(report_pair) not in generated and tuple(reversed(report_pair)) not in generated:
    ...
    if c['adaptive']['enabled']:
        ...
        if not isinstance(c['adaptive']['top_regions'], int) or c['adaptive']['top_regions'] < 1:
            raise ValueError('adaptive.top_regions must be positive')
